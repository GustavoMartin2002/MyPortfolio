"use client";

import Image from "next/image";
import { motion } from "framer-motion";
import { fadeIn } from "@/utils/variant";

export default function HeroServices() {
  return (
    <section className="hero min-h-screen relative mask-t-from-100% mask-b-from-100%">
      <Image
        className="-z-10 object-cover pointer-events-none select-none mask-t-from-70% mask-b-from-70%"
        src="/background-services.webp"
        alt="services.webp"
        fill
        quality={100}
        priority={true}
      />
      <div className="hero-content">
        <motion.div
          variants={fadeIn("up", 0.1)}
          initial={"hidden"}
          animate={"show"}
          className="max-w-3xl text-center"
        >
          <h1
            className="text-5xl text-shadow-lg font-bold linear-gradient-blue
            max-lg:text-4xl max-md:text-3xl max-sm:text-2xl"
          >
            Serviços
          </h1>

          <h2
            className="text-xl text-shadow-lg font-light mt-3 
            max-lg:text-lg max-md:text-base max-sm:text-sm"
          >
            Transformo ideias em soluções digitais que funcionam.<br className="max-sm:hidden" />
            Descubra como a tecnologia pode impulsionar o seu negócio.
          </h2>
        </motion.div>
      </div>
    </section>
  );
}
